...
(function(){
  ...
  if(!matchMedia('(hover:none)').matches){
    ...
    if(c&&d){
      ...
      document.querySelectorAll('a,button,.sw,.place-btn,.nav-ico,.pg-thumbs button').forEach(function(el){el.addEventListener('mouseenter',grow);el.addEventListener('mouseleave',shrink);});
    }
  }
  ...
  if(main){
    document.querySelectorAll('.pg-thumbs button').forEach(function(t){t.addEventListener('click',function(){
      document.querySelectorAll('.pg-thumbs button').forEach(function(o){o.classList.remove('active');});
      t.classList.add('active');
      // La vignette est desormais un <button> : la source est portee par
      // l'image qu'il contient, plus par l'element clique lui-meme.
      var im=t.querySelector('img');
      if(im) main.src = im.dataset.full || im.src;
    });});
  }
